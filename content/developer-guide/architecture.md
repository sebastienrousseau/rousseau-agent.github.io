...
layout: "page"
permalink: "https://docs.rousseau-agent.dev/developer-guide/architecture/"
...
Called on the hot path before every tool call. See [Approval Policies](/user-guide/approval-policies/).
...
See [Compression + Recall](/user-guide/compression-recall/).
...
Trade-off: `claudecli` users must trust `claude`'s permission model. Documented in [Providers: claudecli](/providers/claudecli/).

## Next

- [Add a transport](/developer-guide/add-a-transport/) — how a new interface implementer looks.
- [Add a provider](/developer-guide/add-a-provider/) — same pattern, different interface.
- [Add a tool](/developer-guide/add-a-tool/) — the smallest extension point.

## Related pages

- [Concepts](/concepts/) — high-level tour.
- [Agent loop](/agent-loop/) — the runtime shape.
- [MCP](/mcp/) — external tool exposure.
- [Configuration](/configuration/) — the config surface each interface reads from.
