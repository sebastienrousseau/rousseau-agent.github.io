...
layout: "page"
permalink: "https://docs.rousseau-agent.dev/ja/reference/config-schema/"
...
このページは [設定](/ja/configuration/) のマシン可読な相棒です。プローズリファレンスは **なぜ** を説明します。このページは形状を示します。

以下のスキーマは、`internal/config/config.go` の `Config` 構造体を JSON-Schema 形状で表したビューです。フィールド名、型、デフォルトについては信頼できます — Go 構造体が真実のソースです。
...
### `openai` (`openai`、`openrouter`、`ollama` で共有)
...
解決順 (`internal/config/config.go` から):

1. CLI フラグ (呼び出し側の `viper.BindPFlag` 経由)。
2. 環境変数 (任意のキーに対する `ROUSSEAU_<UPPERSNAKE>`。`ANTHROPIC_API_KEY` は特別ケース)。
3. Config ファイル (デフォルトで `~/.config/rousseau/config.yaml`)。
4. ハードコードされたデフォルト (`internal/config/config.go` の `setDefaults` を参照)。

## 環境マッピング

すべてのフィールドは、`.` を `_` に置き換えて大文字の `ROUSSEAU_<SECTION>_<KEY>` にマップします。例:

| キー | 環境変数 |
|---|---|
| `log.level` | `ROUSSEAU_LOG_LEVEL` |
| `anthropic.api_key` | `ROUSSEAU_ANTHROPIC_API_KEY` (`ANTHROPIC_API_KEY` も) |
| `agent.max_iterations` | `ROUSSEAU_AGENT_MAX_ITERATIONS` |
| `slack.app_token` | `ROUSSEAU_SLACK_APP_TOKEN` |

## 次に

- [設定](/ja/configuration/) — 各ノブの説明付きプローズリファレンス。
- [リファレンス: CLI コマンド](/ja/reference/cli-commands/) — フラグはすべての config フィールドを上書きします。
- [リファレンス: 終了コード](/ja/reference/exit-codes/) — config パースエラーが init システムにどう表面化するか。
