...
layout: "page"
permalink: "https://docs.rousseau-agent.dev/user-guide/tools/"
...
Each is registered via `registry.MustRegister(builtin.NewXTool())`. Register additional tools without touching the agent core — see [Developer Guide: Add a Tool](/developer-guide/add-a-tool/).
...
- The tool has no built-in allowlist. The [Approver](/user-guide/approval-policies/) is the load-bearing gate. **Always** enable pattern-mode approval on unattended daemons.
- The command runs with the daemon's UID and filesystem visibility. Layer a rootless container underneath ([Deployment](/deployment/)).
...
This is the same channel used for approver denials — see [Approval Policies](/user-guide/approval-policies/).
...
- [User Guide: Approval Policies](/user-guide/approval-policies/) — the gate on every tool call.
- [Developer Guide: Add a Tool](/developer-guide/add-a-tool/) — build your own.
- [Concepts](/concepts/) — how tools fit into the agent loop.
- [Agent loop](/agent-loop/) — how tool results feed back into the next turn.
- [Reference: Tool schemas](/reference/tool-schemas/) — machine-readable schemas.
